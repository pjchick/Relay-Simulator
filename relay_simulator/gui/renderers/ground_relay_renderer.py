"""
Ground DPDT Relay component renderer.

Renders a relay with coil (with GND pin) and two sets of contacts (poles).
"""
...
class GroundRelayRenderer(ComponentRenderer):
    """
    Renderer for GroundDPDTRelay components.
    Supports rotation and flipping transformations.
    """
    
    WIDTH = 60   # Relay width in pixels
    HEIGHT = 200  # Relay height in pixels (same as regular DPDT)
    ...
    def render(self, zoom: float = 1.0) -> None:
        """
        Render the ground relay component.
        ...
        if is_energized and self.powered:
            # Brighten body fill slightly when energized
            try:
        ...
        # Centered horizontally, at COIL/GND pin height
        coil_center_x = cx
        coil_center_y = cy - 80 * zoom  # At COIL/GND pin height
